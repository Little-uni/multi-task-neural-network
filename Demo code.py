...
class ASHDataset(Dataset):
    def __init__(self, images, labels=None, transform=transforms.Compose([transforms.ToTensor()])):
        self.X = images
        self.y = labels
    ...
    def __getitem__(self, i):
        data = self.X.iloc[i, :]
        data = np.asarray(data).reshape(1, len(data_feature_scaled.columns), 1)
        result = self.y.iloc[i, :]
        result = np.asarray(result).reshape(1, len(data_label_scaled.columns), 1)
        
        if self.transform:
            data = self.transform(data)
        if self.y is not None:
            return (data.float(), self.transform(result))
        else:
            return data.float()
...
class MultiTaskNeuralNet(nn.Module):
  def __init__(self):
    ...
    self.net = nn.Sequential(
      ...
      nn.BatchNorm1d(6),
      nn.ReLU(inplace=True),
      nn.Linear(6, len(data_label_scaled.columns))
      )
